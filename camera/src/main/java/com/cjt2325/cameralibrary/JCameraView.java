...
//    private static final String TAG = "JCameraView";
    ...
    //录制视频比特率
    public static final int MEDIA_QUALITY_SUPER_HIGH=5*1920*1080;
    ...
    private void initView() {
        ...
        mClose = view.findViewById(R.id.image_close);
        setFlashRes();
    ...
    CheckConfirmListener listener;
    Bitmap frameAtTime;
    String finalVideoPath;
    public void setPlayVideo(Bitmap frameAtTime, String finalVideoPath) {
    ...
    public void setCheckConfirmListener(CheckConfirmListener listener){
        this.listener=listener;
    }

    public void setCloseClickListener(ClickListener closeClickListener) {
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        if (!TextUtils.isEmpty(finalVideoPath)&&null!=frameAtTime) {
            PlayAlbumVideo(firstFrame, finalVideoPath);
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public void setFeatures(int state) {
        if (state==BUTTON_STATE_ONLY_RECORDER){
            mCaptureLayout.setHintVisiable(true);
        }else {
            mCaptureLayout.setHintVisiable(false);
        }
    ...
    public void PlayAlbumVideo(Bitmap firstFrame, final String url){
        new Thread(new Runnable() {
            @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
            @Override
            public void run() {
