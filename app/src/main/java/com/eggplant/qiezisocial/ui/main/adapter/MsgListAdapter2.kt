...
class MsgListAdapter2(data: List<MsgMultiltem<MainInfoBean>>?) : BaseMultiItemQuickAdapter<MsgMultiltem<MainInfoBean>, BaseViewHolder>(data) {

    init {
        addItemType(MsgMultiltem.QUES_MSG, R.layout.ap_msg)
        addItemType(MsgMultiltem.CHAT_MSG, R.layout.ap_msg_chat)
    }

    override fun convert(helper: BaseViewHolder, item: MsgMultiltem<MainInfoBean>) {
        helper.addOnClickListener(R.id.ap_msg_head)
        val newMsgTime = item.bean.newMsgTime
        if (newMsgTime != 0L) {
            when {
        ...
        if (item.bean.msgNum > 0) {
            helper.itemView.ap_msg_hint.visibility = View.VISIBLE
//            helper.itemView.ap_msg_hint.text = item.bean.msgNum.toString()
        } else {
            helper.itemView.ap_msg_hint.visibility = View.GONE
        }
        if (helper.itemViewType == MsgMultiltem.QUES_MSG) {
            helper.itemView.ap_msg_nick.text = item.bean.nick
            Glide.with(mContext).load(API.PIC_PREFIX + item.bean.face).into(helper.itemView.ap_msg_head)
            if (item.bean.qsType=="private"){
//                helper.itemView.ap_msg_reply.visibility=View.VISIBLE
                helper.itemView.ap_msg_reply.text = "给你发了一条私人问题"
            }else {
//                helper.itemView.ap_msg_reply.visibility=View.GONE
                helper.itemView.ap_msg_reply.text = "${item.bean.msg}"
            }
//            helper.itemView.ap_msg_reply.setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0)
//            var list = item.bean.qsUserFace?.split(",")
//            var faceList = list?.filter {
//                it.isNotEmpty()
//            }
//            if ((item.bean.qsid!=0L||item.bean.gsid != 0L) && faceList != null && faceList.isNotEmpty()) {
//                if (faceList.size > 4) {
//                    faceList = faceList.subList(0, 4)
//                }
//                createGropBitmap(mContext, helper.itemView.ap_msg_head, *faceList.toTypedArray())
//                helper.itemView.ap_msg_nick.text = item.bean.qsNick
//            } else {
//                Glide.with(mContext).load(API.PIC_PREFIX + item.bean.face).into(helper.itemView.ap_msg_head)
//            }

            if (item.bean.qsTxt.isNotEmpty()) {
                var qsTxt = item.bean.qsTxt
                helper.itemView.ap_msg_qstxt.text = qsTxt
//                helper.itemView.ap_msg_qstxt.setUrlText(qsTxt)
            }
            val mood = item.bean.mood
        ...
        } else if (helper.itemViewType == MsgMultiltem.CHAT_MSG) {
            if (!TextUtils.isEmpty(item.bean.`object`)) {
                //回答
                helper.itemView.ap_msg_nick.text = item.bean.msg
                //问题
                helper.itemView.ap_msg_reply.text = item.bean.`object`
