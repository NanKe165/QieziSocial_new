...
public class NineGridView extends ViewGroup {
    ...
    private int MODE_GRID_gridWidth;
    private int MODE_GRID_gridHeight;
...
//                gridWidth = gridHeight = (totalWidth - gridSpacing * (columnCount - 1)) / columnCount;
                ...
                Log.i("nineview","gridW"+gridWidth);
                if (mode == MODE_GRID && mImageInfo.size() % 2 == 0 && mImageInfo.size() % 3 != 0) {
                    MODE_GRID_gridWidth = (totalWidth - gridSpacing) / 2;
                    MODE_GRID_gridHeight = ((totalWidth - gridSpacing) / 2);
                    Log.i("nineview","MODE_GRID gridW"+MODE_GRID_gridWidth);
                }

                if (!isSquare) {
                    gridHeight -= 60;
                    MODE_GRID_gridHeight-=60;
                }
            ...
            if (mode == MODE_GRID && mImageInfo.size() % 2 == 0 && mImageInfo.size() % 3 != 0) {
                height = MODE_GRID_gridHeight * rowCount + gridSpacing * (rowCount - 1) + getPaddingTop() + getPaddingBottom();
            }
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            if (mode == MODE_GRID && mImageInfo.size() % 2 == 0 && mImageInfo.size() % 3 != 0) {
                 left = (MODE_GRID_gridWidth + gridSpacing) * columnNum + getPaddingLeft();
                 top = (MODE_GRID_gridHeight + gridSpacing) * rowNum + getPaddingTop();
                 right = left + MODE_GRID_gridWidth;
                 bottom = top + MODE_GRID_gridHeight;
            }
            childrenView.layout(left, top, right, bottom);

            if (mImageLoader != null) {
                mImageLoader.onDisplayImage(getContext(), childrenView, mImageInfo.get(i).thumbnailUrl, null);
            }
        }
    }

    public void isSquare(boolean isSquare) {
    ...
    public interface ImageLoader {
         ...
         * @param longImg
         */
        void onDisplayImage(Context context, ImageView imageView, String url, SubsamplingScaleImageView longImg);
