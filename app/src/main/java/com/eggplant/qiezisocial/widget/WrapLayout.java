...
public class WrapLayout extends ViewGroup {
     ...
     * @param mb       下外边距
     */
    public void setData(String[] data, Context context, int textSize, int textcolor, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb) {
        createChild(data, context, textSize, textcolor, pl, pt, pr, pb, ml, mt, mr, mb);
    }

    public void setData(List<String> data, Context context, int textSize, int textcolor, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb) {
        removeAllViews();
        String[] mydata = null;
        if (data != null) {
        ...
        setData(mydata, context, textSize, textcolor, pl, pt, pr, pb, ml, mt, mr, mb);
    }

    private void createChild(String[] data, final Context context, int textSize, int textcolor, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb) {
...
//                lineHeight = Math.max(lineHeight, childHeight); //记录行高
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childWidth +leftMargin + rightMargin + lineWidth > width)  //大于父布局的宽度
            {
                // 记录这一行所有的View以及最大高度
            ...
            lineHeight = Math.max(lineHeight, childHeight + topMargin
                    + bottomMargin);
            lineViews.add(child);
        }
        ...
        for (int i = 0; i < lineNums; i++) {
            ...
            for (int j = 0; j < lineViews.size(); j++) {
                ...
                LayoutParams layoutParams = lineChild.getLayoutParams();
                int leftMargin=0,rightMargin=0,topMargin=0,bottomMargin=0;
                if (layoutParams instanceof MarginLayoutParams) {
                    MarginLayoutParams lp = (MarginLayoutParams) lineChild.getLayoutParams();//得到属性参数
                    leftMargin=lp.leftMargin;
                    rightMargin=lp.rightMargin;
                    topMargin=lp.topMargin;
                    bottomMargin=lp.bottomMargin;
                ...
                left += lineChild.getMeasuredWidth() +rightMargin + leftMargin;
            }
            left = 0;//将left归零
            top = lineHeight;
        }
